...
|**/Platform:[*platform type*]**|Forces the given platform to be used, instead of the platform determined from the current runtime. This option is able to force only x86, and x64 platforms on Windows. The ARM option is broken and ARM64 is missing.<br />Valid values are x86, x64, and ARM.<br /> 
